...
input_file = 'dataset_instagram-reel-scraper_2023-04-20_00-58-40-725 (2).csv'
...
output_csv = file_path + 'reelsOutput.csv'
...
def stm_check(columns, userName, dataRow):

    # This variable identifies the campaign hashtag.
    # Current assumption is there will only be one campaign hashtag
    # at a time.
    #camp_hashtag = "selftapemaylotr"
    stm_hashtag = "selftapemay"
...
columns_to_keep = ['id', 'ownerFullName', 'ownerUsername', 'timestamp','productType', 'campaignFlag', 'url', '_id', '_createdDate', '_updatedDate', '_owner']
...
with open(input_csv, 'r', encoding=encoding) as input_file:
    ...
    with open(output_csv, 'w', newline='', encoding=encoding) as output_file:
        ...
        for i, row in enumerate(reader):
            ...
            # Call the stm check function to see if user's post has the proper qualifier hasthag (#selftapemay)
            stmhash_check = stm_check(hash_columns, output_row['ownerUsername'], {key: row[key] for key in hash_columns})
            ...
            if output_row['timestamp'] != '' and stmhash_check:

                if start_dttm <= timestamp_dt <= end_dttm and output_row['productType'] == 'clips':
